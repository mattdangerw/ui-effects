...
def main2():
    ...
    stage.get_default()
    color = clutter.Color()
    color.from_string("#BBCCFF")
    stage.set_color(color)
    stage.set_size(SCREEN_W, SCREEN_H)
    stage.show()
    
    template = clutter.Effect()
    # random_ripple()
      
    stage.connect("key-press-event", lambda w: clutter.main_quit())
...
def main():
    ...
    color = clutter.Color()
    color.from_string("blue")
    rect2.set_border_color(color)
